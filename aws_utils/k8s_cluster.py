...
def _build_config(specs: Dict[str, Union[str, int, float]], environment: Mapping):
    ...
    _kops_cmd(['create', 'cluster', environment['NAME'], '--cloud=aws', '--zones=us-east-1a',
               '--dry-run', '-o=yaml'], environment, outfile=manifest_file)
...
def k8s_cluster(specs: Dict[str, Union[str, int, float]]):
    ...
    if not mgmt._s3_kops_config_exist(bucket=environment['KOPS_STATE_STORE']):
        mgmt._s3_setup(name=environment['KOPS_STATE_STORE'], region=specs['region'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nodes', type=int, default=2,
                        help='The number of masters. Defaults to 2.')
    ...
    print(_edit_config(cluster_specs))
